...
class EndToEndModel:
    ...
    def __init__(self, vocabnum, ClassEmbeddingsize, Params=None, seqlens = 15, sess = None, LogDir = None, ckpt_dir = None, seed = None, WriteEmbeddingVis=False,socket=None):
        if Params != None:
            self.Params = Params
            self.socket = socket
        ...
        for i in range(self.Params["NumofModels"]):
            self.socket.sleep(0)#keep socket alive while deepkinzero is working.so client wont disconnect.
            Graph = tf.Graph()
