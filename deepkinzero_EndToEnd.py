...
def Write_predictions_server(probabilities, Sub_IDs, Sequences, Residues, Candidatekinases, Candidate_UniProtIDs, top_n, sub_ID_has_original_rows=False):
    allData = []
    allData.append('Row id in the Original File, Phosphosite Residue, Substrate protein, Phosphosite Sequence' + ',' + ','.join(['Predicted Kinase UniProt ID', 'Predicted Kinase Name', 'Predicted Probability'] * top_n))
    original_row = 0
...
def Run(Model = "ZSL",
        ...
        CheckpointPath=None,socket=None,socketId=None,resultId=None,results=None):
    ...
    print("Creating Kinase Embeddings")
    #send deepkinzero working status to client.
    socket.sleep(0)
    socket.emit('status', "Creating Kinase Embeddings", room=socketId)
    socket.emit('resultId',{'resultId':resultId})
    ...
    if TestData != []:# normally this is TestData != '' but data is coming from client...(website)
        TestDS = dataset()
        ...
        TestDS.getdataServer(TestData, KE, islabeled=TestisLabeled,MultiLabel=True) #TestData must be array of arrays.each element is a row as array
    ...
    print("Creating Data Embedding model and training it. ")
    socket.emit('status', "Creating Data Embedding model and training it.", room=socketId)
    socket.sleep(0)
    EndToEndmodel = EndToEndModel(vocabnum = Phosphosite_Seq_Size[1], seqlens = Phosphosite_Seq_Size[0], Params=ModelParams, LogDir = FolderName, ckpt_dir = DEFolder, WriteEmbeddingVis=False, ClassEmbeddingsize=KE.Embedding_size, seed=seed,socket=socket)
    if LoadModel:
    ...
    if TrainData != '':
        ...
        socket.emit('status', "Predicting...", room=socketId)
    ...
    if TestData != '':
        ...
        socket.sleep(0)
        socket.emit('status', "Retrieving prediction results...", room=socketId)

        allData = Write_predictions_server( probabilities, TestDS.Sub_IDs, TestDS.Sequences, TestDS.Residues, Candidatekinases, Candidate_UniProtIDs, top_n=Top_n, sub_ID_has_original_rows=False)
        #socket.emit('result', {'resultData': allData})
        results[resultId] = allData
        return allData
    if TrainData != '':
